...
class PatchEmbedding(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        # No Repeat layer in einops.layers. Can define it, or use torch.nn.parameter.Parameter.repeat
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, mask: Optional[torch.Tensor] = None):
        ...
        values = self.rearrange_heads(self.key(x))
        ...
        out = self.projection(self.rearrange_out(out))
...
class TransformerEncoderBlock(nn.Sequential):
    def __init__(
        self, emb_size=768, drop_p=0.0, forward_expansion=4, forward_drop_p=0.0
    ):
...
# To do:
# 1. replace rearrange, einsum, repeat with layers
# 2. annotated with torch.jit.export for functions to be exported
# 3. ----